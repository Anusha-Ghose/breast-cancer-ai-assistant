...
async def extract_entities(raw_text: str, document_type: str) -> dict:
    ...
    prompt = PromptTemplate(
        template="""You are an expert clinical data and handwritten prescription extractor. Your task is to accurately extract ALL patient, diagnostic, and prescription information from the provided medical document text.
        ...
        {format_instructions}""",
        input_variables=["document_type", "raw_text"],
    ...
    try:
        result = chain.invoke({"document_type": document_type, "raw_text": raw_text})
        return result
